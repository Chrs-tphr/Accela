...
//INSERT TEST CODE START
	...
	//get cap by status
	//get inspections
	//if no inspections skip else get inspection info
	//get result status
	//if result status is not "Not Inspected" or "Not Ready" get result status date else skip inspection
	//add date to array
	//if more inspections start over else get latest date from array
	
	var id = getAppSpecific("Permit Issued",capId);
	var ed = getAppSpecific("Permit Expiration",capId);
	logDebug("id: "+id+", ed: "+ed);
	
	if(!matches(id,null,"")){
		if(!matches(id,null,"") && matches(ed,null,"")){
			editAppSpecific("Permit Expiration", dateAdd(ed,181) ,capId);
		}
			...
			if(capInspArr.length){
				...
				if(lastValidInspResultDateTime){
					if(getAppSpecific("Permit Expiration",capId) == dateAdd(new Date(lastValidInspResultDateTime), 181)){
						...
						logDebug("Updating Permit Expiration from: "+getAppSpecific("Permit Expiration",capId)+", to: "+dateAdd(new Date(lastValidInspResultDateTime),181));
						editAppSpecific("Permit Expiration", dateAdd(new Date(lastValidInspResultDateTime), 181));
					}
				}
			}else{
				logDebug("No inspections on cap");
			}
		}else{
			logDebug("Could not get inspection sObj for cap ");
		}
	}else{
		logDebug("Permit is not issued");
	}
